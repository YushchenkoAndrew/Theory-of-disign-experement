...
public class DesignExperiment {
    ...
    private int k = 3;
    private double l = java.lang.Math.sqrt(k);


    private int m = 3;
    private int N = 14;

    private double[][] X = new double[N][10];
    private double[][] XPlan = new double[N][10];

    // Given coefficient for equation y(i)
    //                      0   x1      x2      x3      x12     x13     x23     x123    x1^2    x2^2    x3^2
    private double[] A = {1.9,  7.1,    0.4,    3.6,    7.4,    0.2,    2.7,    3,      3.8,    0.9,    2.8};

    //                            min     max
    private int[] X1 = new int[]{  10,    60};
    private int[] X2 = new int[]{ -25,    10};
    private int[] X3 = new int[]{  10,    15};
    ...
    private void fillMatrix_X() {
        for (int i = 0; i < 8; i++) {
            ...
            XPlan[i][0] = X[i][0] == 1 ? X1[1] : X1[0];
            XPlan[i][1] = X[i][0] == 1 ? X2[1] : X2[0];
            XPlan[i][2] = X[i][0] == 1 ? X3[1] : X3[0];
        ...
        XPlan[8][0] += (-l) * (X1[1] - (X1[0] + X1[1]) / 2.0);
        XPlan[9][0] += l * (X1[1] - (X1[0] + X1[1]) / 2.0);

        XPlan[10][1] += (-l) * (X2[1] - (X2[0] + X2[1]) / 2.0);
        XPlan[11][1] += l * (X2[1] - (X2[0] + X2[1]) / 2.0);

        XPlan[12][2] += (-l) * (X3[1] - (X3[0] + X3[1]) / 2.0);
        XPlan[13][2] += l * (X3[1] - (X3[0] + X3[1]) / 2.0);
    ...
    public void start() {
        ...
        System.out.println("\t\tx1\t\tx2\t\t\tx3\t\tx12\t\t\tx13\t\tx23\t\tx123\t\tx1^2\t x2^2\t\tx3^2");
        CorrectionPrint.printArray(X);
        System.out.println("\nXplan:");
    ...
    private void fillMatrix_Y(int m) {
        Y = new double[N][m + 1];
        ...
        for (int i = 0; i < Y.length; i++) {
            ...
            for (int j = 0; j < m; j++) {
                Y[i][j] = A[0];
                for (int k = 0; k < XPlan[0].length; k++)
                    Y[i][j] += XPlan[i][k] * A[k + 1] + java.lang.Math.random() * 10 - 5;
                average += Y[i][j];
            }
            //  Save average value
            Y[i][m] = average / m;
